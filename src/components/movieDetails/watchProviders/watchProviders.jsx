...
const WatchProviders = (props) => {
    ...
    const WatchProvider = () => {
        ApiCall(`movie/${Movieid}/watch/providers`).then((Response) => setWatchProviders({...Response.results})); 
      }
    useEffect(() => {
        WatchProvider();
    ...
    <div className="watchProviders">
            ...
            {watchProviders && watchProviders.IN && (watchProviders.IN.flatrate || watchProviders.IN.rent || watchProviders.IN.buy) ? (
              <>
                ...
                {watchProviders && watchProviders.IN && (watchProviders.IN.flatrate || watchProviders.IN.rent || watchProviders.IN.buy) && (
                <p>You can now watch {movieData &&(movieData.title || movieData.name)} online on the following streaming providers in India:</p>
                )}
                ...
                {watchProviders && (watchProviders.IN || !watchProviders.IN) && !watchProviders.IN.flatrate && !watchProviders.IN.rent && !watchProviders.IN.buy && (
                  <p>No watch providers available for {movieData &&(movieData.title || movieData.name)} in India.</p>
                )}
